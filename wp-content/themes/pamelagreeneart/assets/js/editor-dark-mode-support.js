/* global pamelagreeneartIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'pamelagreeneart-supports-dark-theme' ) ) {
	pamelagreeneartDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function pamelagreeneartDarkModeEditorInit( attempt ) {
	...
	if ( pamelagreeneartIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						pamelagreeneartDarkModeEditorInit( attempt + 1 );
					},
