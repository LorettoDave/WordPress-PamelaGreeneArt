/* global pamelagreeneartGetHexLum, jQuery */
( function() {
	// Add listener for the "background_color" control.
	wp.customize( 'background_color', function( value ) {
		value.bind( function( to ) {
			var lum = pamelagreeneartGetHexLum( to ),
				isDark = 127 > lum,
				...
				stylesheetID = 'pamelagreeneart-customizer-inline-styles',
				stylesheet,
				styles;
			...
			if ( ! stylesheet.length ) {
				jQuery( '#pamela-greene-art-style-inline-css' ).after( '<style id="' + stylesheetID + '"></style>' );
				stylesheet = jQuery( '#' + stylesheetID );
			}
